...
############################################
###                                      ###
### input parameters read from yaml file ###
###                                      ###
############################################
def readYML(file):        
...
def main():
    #default value for yaml file (can be given as command line argument
    yamlfile ="simulationsConfig.yml" # CHANGE THIS DEBUGKEBAB BEFORE COMMIT
    ...
    try:
       ...
       print('ERROR, usage: .py --i=<input yaml file>')
       sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
