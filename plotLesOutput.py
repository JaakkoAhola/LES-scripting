...
if __name__ == "__main__":
    ...
    namelist = os.path.dirname(os.path.realpath(tiedostolista[0]))+"/NAMELIST"
    print("NAMELIST", namelist)

    if os.path.isfile(namelist):
        for spinupNamelistMuuttuja in 'Tspinup', 'lsauto%delay':
            try:
...
def piirra_aikasarjasettii( muuttuja, muuttuja2 = None, muunnosKerroin2 = 1.0, variKartta = plt.cm.gist_rainbow, variRefVektori = jarjestys, colorBar = None, colorBarTickValues = [0,1], colorBarTickNames = ['0','1'], muunnosKerroin = 1.0, longName = 'titteli', xlabel = 'time [h]', ylabel='ylabel', extendBelowZero = True, asetaRajat = True, ymin = None, ymax = None, relative = False, savePrefix = None, omaVari = True, tit = ' ', nollaArvo = None, xlabels = None, ylabels = None, xticks = None, yticks = None, spinup = None, ylabelFont = None, askChangeOfVariable = False, piilotaOsaXlabel = False, legenda = True, profiili = False, piilotaOsaYlabel = False, piilotaOsaYlabelParam = 3, NCtiedosto= False, tallenna = False ):
    ...
    for i in range(len(tiedostolista)):
        ...
        try:
            ...
            print("KeyError, variable " + muuttuja + " doesn't exist in " + tiedostonimi[i] )
            continue
...
def piirra_profiiliKehitys(  muuttuja, variKartta = plt.cm.gist_rainbow, colorBar = None, colorBarTickValues = [0,1], colorBarTickNames = ['0','1'], muunnosKerroin = 1.0, longName = 'titteli', xlabel = 'xlabel', ylabel='ylabel', extendBelowZero = True, asetaRajat = True, ymin = None, ymax = None, xmin = None, xmax = None,  savePrefix = None, aikaPisteet = None, tit = ' ', nollaArvo = None, rajaKerros = None, paksuus = None, xlabels = None, ylabels = None, xticks = None, yticks = None, tempConversion = False, askChangeOfVariable = False,fontsizeCustom = False, viivaTyyli = None, plottaaKaikkiSamaan = False, useSnsColor = False, xTikit = None, oikeatXtikit = None ):
    ...
    for i in reversed(range(len(tiedostolista))):
        if plottaaKaikkiSamaan and i == 0:
            uusikuva = True
        elif plottaaKaikkiSamaan:
            uusikuva = False
        ...
        for t in range(len(aikaPisteet)):
            ...
            if not plottaaKaikkiSamaan:
                if t == 0: uusikuva = True

            try:
                muuttuja_data      = mdp.read_Data( filenamePS[i], muuttuja )*muunnosKerroin
            except KeyError:
                print("KeyError, variable " + muuttuja + " doesn't exist in " + filenamePS[i] )
                noErrors = False
                break
            ...
            if aikaP >0:
                ...
                plt.axhline(  top[aikaP], color = color, linestyle= (0, (1, 5)) )aikaP 0 t 0 i 1
        ...
        if noErrors:
            ...
            if saveFig and (not plottaaKaikkiSamaan):
                valitagi = '_'
                if not plottaaKaikkiSamaan:
                    if customLabels:
    ...
    if saveFig and plottaaKaikkiSamaan:
...
def piirra_domainProfiili( muuttuja, muunnosKerroin = 1.0, transpose = False, longName = None , savePrefix = None, useDN = False, colorBarTickValues = None, colorBarTickNames = None, colorBarTickValuesInteger = False,  xlabels = None, ylabels = None, xticks = None, yticks = None, variKartta = plt.cm.Blues, profiili = False, radCool = False, cloudBaseTop = False, spinup = None, testi = False, sliceXbeg = None, sliceXend = None, sliceYbeg = None, sliceYend = None, askChangeOfVariable = False, fontsizeCustom = False ):
    ...
    for i in range(len(tiedostolista)):
        ...
        if colorBarWorking:
            print("colorBarTickValues",colorBarTickValues)
            cbar = fig.colorbar(cax, ax=ax, ticks = colorBarTickValues, norm = norm)#,fraction=0.046, pad=0.04 ###, shrink=.383, pad=.03, aspect=10,
        ...
        if saveFig:
            ...
            if not EMUL:
                plt.savefig( kuvakansio + savePrefix + '_' + muuttuja + '_' + valitagi.replace(" ","_") + saveTag + LVLprintSave + '.png')
...
"#800000" #Maroon
    ...
    if int(lvl) == -8: # Ovchinnikov style isdac plots
        ...
        piirra_aikasarjasettii( muuttuja = 'Nc_ic', muunnosKerroin = 1.e-6, longName = 'in-cloud CDNC', ylabel = r'[$\#/10^{-6}kg$]', ymin = 0.0,  savePrefix = 'cdnc' + tagii + '_uclales-salsa' , omaVari = False, xlabel = 'time [h]', spinup = spinup, piilotaOsaXlabel = piilotaOsaXlabel, legenda = legendaPaper  )

        mdp.plot_suljetus(naytaPlotit)

        piirra_aikasarjasettii( muuttuja = 'Ni_ii', muunnosKerroin = 1.e-3, longName = 'Ice number concentration', ylabel = '[#/]', ymin = 0.0,  savePrefix = 'Ni' + tagii + '_uclales-salsa' , omaVari = False, xlabel = 'time [h]', spinup = spinup, piilotaOsaXlabel = piilotaOsaXlabel, legenda = legendaPaper  )
        ...
        WPlabels = list(map(str, WPticks))
        piirra_aikasarjasettii( muuttuja = 'rmH2Oic',     muunnosKerroin = 1.e6,  longName = ' ', ylabel = r'[$10^{-6} kg m^{-2} s^{-1}$]', ymin = 0.0, ymax=WPticks[-1], extendBelowZero = False,  savePrefix = 'depIce', omaVari = False, xlabel = 'time [h]', yticks = WPticks, ylabels = WPlabels, spinup = spinup, piilotaOsaXlabel = piilotaOsaXlabel, piilotaOsaYlabel = False, piilotaOsaYlabelParam = 5, askChangeOfVariable = askChangeOfVariable, legenda = legendaPaper )
        ...
        ["DU_ii", "Dust mass mixing ration in ice", "kg/kg", 1.e12]]:
        ...
        print("origFig",origFig)
        mpl.rcParams['figure.figsize'] = [mpl.rcParams['figure.figsize'][0], mpl.rcParams['figure.figsize'][1]*1.4]
        print("mpl.rcParams['figure.figsize']",mpl.rcParams['figure.figsize'])
        aikaPisteet      = xTicksSeconds[::4] # even hours
        aika_color       = [ sns.color_palette(kartta, len(aikaPisteet)) for kartta in [ "Greens", "Blues", "Reds"]]
        listOfLineStyles = [ (0, ()), (0, (5, 1)), (0, (5, 5)), (0,(3, 1, 1, 1)), (0,(3, 5, 1, 5))] #,  (0, (1, 5))
        #aikaBAR         = varibaari( aikaPisteet,  aika_color)
        #aikaTIT         = 'time [h]'
        #cbvalTStr       = list(map(str, xTicksSeconds))

        piirra_profiiliKehitys( 'theta',  variKartta = aika_color, longName =  'Potential temperature', xlabel = r'$\theta$' + ' [K]', ylabel = 'z [m]', savePrefix = 'theta_evol', aikaPisteet = aikaPisteet, asetaRajat = False, viivaTyyli = listOfLineStyles, plottaaKaikkiSamaan = True, useSnsColor = True )

        piirra_profiiliKehitys( 'theta',  variKartta = aika_color, longName =  'Potential temperature', xlabel = r'$\theta$' + ' [K]', ylabel = 'z [m]', savePrefix = 'theta_evol', aikaPisteet = aikaPisteet, asetaRajat = False, useSnsColor = True, xTikit = range(264, 273), xmin = 265, xmax = 273.5 )
