...
def setPlottingContext(printing = False):
...
def piirra_aikasarjasettii( muuttuja, muuttuja2 = None, muunnosKerroin2 = 1.0, variKartta = plt.cm.gist_rainbow, variRefVektori = jarjestys, colorBar = None, colorBarTickValues = [0,1], colorBarTickNames = ['0','1'], muunnosKerroin = 1.0, longName = 'titteli', xlabel = 'time [h]', ylabel='ylabel', extendBelowZero = True, asetaRajat = True, ymin = None, ymax = None, relative = False, savePrefix = None, omaVari = True, tit = '', nollaArvo = None, xlabels = None, ylabels = None, xticks = None, yticks = None, spinup = None, ylabelFont = None, askChangeOfVariable = False, piilotaOsaXlabel = False, legenda = True, profiili = False, piilotaOsaYlabel = False, piilotaOsaYlabelParam = 3, piilotaOsaXlabelParam =4, NCtiedosto= False, tallenna = False, loc = 3, conversionWithAirdensity = False, verticalLineList = None, removeNeg = False ):
    ...
    if plottausOnnistuu:
        ...
        if piilotaOsaYlabel:
            ...
            k = 0
            for line in ax.yaxis.get_ticklines()[0::2]:
                #print(line, k, xLabelListMajorLineBoolean[k])
                if np.mod(k,piilotaOsaYlabelParam) != 0:
                    line.set_markersize(mpl.rcParams["xtick.minor.size"])
                k= k + 1
...
def piirra_profiiliKehitys(  muuttuja, variKartta = plt.cm.gist_rainbow, colorBar = None, colorBarTickValues = [0,1], colorBarTickNames = ['0','1'], muunnosKerroin = 1.0,\
    ...
    for i in reversed(range(len(tiedostolista))):
        ...
        if useLog:
            muuttuja_data = np.log10(muuttuja_data)
        ...
        if not cloudEvol:
            ...
            #plt.axhline(  top[topmax],   color = "black", linestyle= "dashed", zorder = 0 )
            #plt.axhline(  base[basemin], color = "black", linestyle= "dashed", zorder = 0 )
...
#            profColormap[0]="white"
            ...
            tstart = -17
            tend = -9
            a = np.arange(tstart, tend)
            b = np.arange(tstart+np.log10(5), tend)
            lev = np.sort(np.concatenate((a,b)))
            
            for mm in ["P_cDUa", "P_cDUc", "P_cDUi"]:
                contour.prof(mm, title = "",
                             yticks = cloudTicks,
                             timeEndH= 33.05, levels = lev,
                             useColorBar = False, showXaxisLabels=False, 
                             showXLabel = False,
                             folder = datakansio, figurePrefix = labelArray[ind]+ "_", kuvakansio = picturefolder )
        
            
            contour.prof("P_cDUi", title = "",
                         yticks = cloudTicks,
                         timeEndH= 33.05, levels = lev,
                         useColorBar = True,
                         folder = datakansio, figurePrefix = "_".join( ["colorbar", labelArray[ind]] ), kuvakansio = picturefolder )
...
#                         kuvakansio = picturefolder, colors = profColormap)
        ...
        if kokojakauma:
            piirra_kokojakauma( muuttujaR = 'S_Rwiba', muuttujaN = 'S_Niba', typename = 'Ice', korkeusH = 700, aikaT = 6, ymax = 11000, xmax = 1.6, yCustSize = 2, xCustSize = 2, sekoita = False, interplo = False, askChangeOfVariable = askChangeOfVariable ) # , xCustSize = 2
            piirra_kokojakauma( muuttujaR = 'S_Rwiba', muuttujaN = 'S_Niba', typename = 'Ice', korkeusH = 400, aikaT = 6, ymax = 11000, xmax = 1.6, yCustSize = 2, xCustSize = 2, sekoita = False, interplo = False, askChangeOfVariable = askChangeOfVariable ) # , xCustSize = 2
            piirra_kokojakauma( muuttujaR = 'S_Rwiba', muuttujaN = 'S_Niba', typename = 'Ice', korkeusH = 200, aikaT = 6, ymax = 11000, xmax = 1.6, yCustSize = 2, xCustSize = 2, sekoita = False, interplo = False, askChangeOfVariable = askChangeOfVariable )
        ...
        if kehitys:
            ...
            if tmax > 36*3600:
                #aikaPisteet = xTicksSeconds[::16][:-2] #
                aikaPisteet = np.asarray([ 2, 4, 16, 32])*3600
            else:
            ...
            riXTicks = np.arange(0, 25+1,5)*1e-3
            riXLabels = list(map(str, riXTicks))
            riXLabels[0] = "0"
            colorpaletti =  sns.color_palette("hls", len(aikaPisteet))
            piirra_profiiliKehitys( 'P_ri', muunnosKerroin = muunnosTemp,  variKartta = colorpaletti, longName =  None,\
                                   xlabel = r'$\mathbf{('+ unitTemp+ ')}$', ylabel = r'$\mathbf{height {\ } (m)}$',\
                                   savePrefix = 'evol03_P_ri', aikaPisteet = aikaPisteet, asetaRajat = False, useSnsColor = True,  viivaTyyli = listOfLineStyles,\
                                   xticks = riXTicks, xlabels =riXLabels, yticks = cloudTicks, legenda = legendaPaper, cloudEvol = False, excludeNeg = True  )
            mdp.plot_suljetus( not naytaPlotit)
            
            rlXTicks = np.arange(0, 200+1,50)*1e-3
            rlXLabels = list(map(str, rlXTicks))
            rlXLabels[0] = "0"
            piirra_profiiliKehitys( 'P_rl', muunnosKerroin = muunnosTemp,  variKartta = colorpaletti, longName =  None,\
                                   xlabel = r'$\mathbf{('+ unitTemp+ ')}$', ylabel = r'$\mathbf{height {\ } (m)}$',\
                                   savePrefix = 'evol04_P_rl', aikaPisteet = aikaPisteet, asetaRajat = False, useSnsColor = True,  viivaTyyli = listOfLineStyles,\
                                   xticks = rlXTicks, xlabels=rlXLabels, yticks = cloudTicks, legenda = legendaPaper, cloudEvol = False, excludeNeg = True )
            
            mdp.plot_suljetus( not naytaPlotit)
            xticksLog = np.arange(-5, 2.5 + 0.1, 2.5)
            xlabelsLog = [r"$10^{" + str(elem) + "}$" for elem in xticksLog]
            piirra_profiiliKehitys( 'nucl_ni', useLog=True,  variKartta = colorpaletti, longName =  None,\
                                   xlabel = r'$\mathbf{(m^{-3}\ s^{-1})}$', ylabel = r'$\mathbf{height {\ } (m)}$',\
                                   savePrefix = 'evol05_nucl', aikaPisteet = aikaPisteet, asetaRajat = False, useSnsColor = True,  viivaTyyli = listOfLineStyles,\
                                   xticks = xticksLog, xlabels=xlabelsLog, yticks = cloudTicks, legenda = legendaPaper, cloudEvol = False, excludeNeg = True )
...
#                                        variKartta = mdp.scientific_colormaps("turku", reverse = True), spinup = spinup, piilotaOsaYlabelParam =  piilotaOsaYlabelParam)
            ...
            for heatmapDict in [{"muuttuja":"P_cDUa", "name":"Total mass mixing ratio of dust in aerosols",\
                                           "savePrefix":"colorbar","unit": mixratunit,\
                                           "useColorBar":True, "colorBarOnly":True, "colorBarMainLabel" : r'$\mathbf{(10^{-12}' + mixratunit +")}$",\
                                           "showXlabel": False},\
                                {"muuttuja":"P_cDUa", "name":"Total mass mixing ratio of dust in aerosols",\
                                           "savePrefix":None, "unit": mixratunit,\
                                           "useColorBar":False, "colorBarOnly":False, "colorBarMainLabel" : None,\
                                           "showXlabel": True},\
                                {"muuttuja":"P_cDUc", "name":"Total mass mixing ratio of dust in cloud droplets",\
                                           "savePrefix":None, "unit": mixratunit,\
                                           "useColorBar":False, "colorBarOnly":False, "colorBarMainLabel" : None,\
                                           "showXlabel": True},\
                                {"muuttuja":"P_cDUi", "name":"Total mass mixing ratio of dust in ice",\
                                           "savePrefix":None, "unit": mixratunit,\
                                           "useColorBar":False, "colorBarOnly":False, "colorBarMainLabel" : None,\
                                           "showXlabel": True}]:
                ...
                piirra_domainprofiiliHeatMap( heatmapDict["muuttuja"], savePrefix = heatmapDict["savePrefix"], longName="", muunnosKerroin = 1e12, yTickLabels = cloudTicks, \
                                             variKartta = sns.color_palette("Reds", colorBarTickTotalNumber), spinup = spinup,\
                                             colorBarTickValues = range(0,colorBarTickTotalNumber+1), useColorBar = heatmapDict["useColorBar"],\
                                             legenda = legendaPaper, hideColorBarLabels = 2, showXlabel = heatmapDict["showXlabel"], \
                                             colorBarOnly = heatmapDict["colorBarOnly"], colorBarMainLabel = heatmapDict["colorBarMainLabel"], piilotaOsaYlabelParam =  piilotaOsaYlabelParam )
                mdp.plot_suljetus( not naytaPlotit)
